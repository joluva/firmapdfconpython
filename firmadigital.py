...
signature_image = "/home/jorge/proyectos/firmapdfconpython/signature_image/whatsapp.jpg"  # Ruta de la imagen de la firma
position = (400, 100)               # Posición (x, y) de la firma en la página (en puntos)
...
def add_signature_to_pdf(input_pdf_path, output_pdf_path):
    ...
    for page_num, page in enumerate(original_pdf.pages):
        # Crear un PDF temporal con la firma en esta página
        packet = BytesIO()
        c = canvas.Canvas(packet, pagesize=letter)
        c.drawImage(signature_image, position[0], position[1], width=100, height=50)
        c.save()
        ...
        # Fusionar la página original con la firma
        page.merge_page(signature_pdf.pages[0])
        writer.add_page(page)
